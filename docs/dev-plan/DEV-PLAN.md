...
- [ ] 🔴 选定设计工具：**Figma**（推荐 / 协作好）/ Sketch / Penpot
- [ ] 🔴 创建团队工作区与项目文件
- [ ] 🔴 建立设计文件结构：HiVideo / HiPixel / Shared 三个 Page
- [ ] 🔴 引入 Apple 官方 macOS UI Kit（Figma Community）
- [ ] 🔴 引入 SF Symbols 图标库
- [ ] 🟡 配置 Figma Tokens 插件
- [ ] 🟡 建立设计版本管理规范（命名 / 备注 / 评审标记）

### 0.5.2 设计 Token 与组件库（Week 1, Day 3-7）

- [ ] 🔴 **配色 Token**：浅色 / 深色双套（默认深色优先调教）
- [ ] 🔴 字体 Token：SF Pro / SF Pro Rounded（数字）/ SF Mono（代码）
- [ ] 🔴 圆角 Token：6px / 8px / 12px / 18px
- [ ] 🔴 间距 Token：4 / 8 / 12 / 16 / 20 / 24 / 32 / 48
- [ ] 🔴 阴影 / 模糊 Token：NSVisualEffectView 各材质映射
- [ ] 🔴 动效 Token：spring 时长 / damping
- [ ] 🔴 基础组件：按钮 / 输入框 / 滑条 / 开关 / 选择器 / Tab / Toast / Modal
- [ ] 🟡 业务组件：海报卡 / 角色头像 / 时间轴片段 / 进度环 / 节点状态行 / 副本选择行
- [ ] 🟡 状态变体：默认 / hover / active / disabled / loading
...
- [x] 🔴 角色侧边栏 + 角色网格主区（**已在 PRD 阶段完成**）
- [x] 🔴 角色详情页（Tony Stark 高保真稿，**已完成**）
- [ ] 🟡 未知角色命名弹窗
- [ ] 🟡 误聚类拆分流程
- [ ] 🟡 对手戏 / 高光集锦页
...
- [ ] 🔴 **副本选择弹窗**（已有原型，正稿化）
- [ ] 🔴 **删除安全网 toast**（5s 撤销 + 30 分钟历史栈）
- [ ] 🔴 **画质增强浮窗**（滤镜列表 + 强度 + 性能仪表）
- [ ] 🔴 **音频增强浮窗**（电平 + LUFS + 设备预设）
- [ ] 🔴 **算力节点设置页**（节点列表 + 连接向导 + 状态详情）
- [ ] 🔴 **添加 SMB 网盘向导**（4 步弹窗 + 测试连接）
- [ ] 🟡 **字幕编辑器**（三栏：列表 / 预览 / 波形）
- [ ] 🟡 **在线字幕搜索结果列表**
- [ ] 🟡 网盘诊断面板
- [ ] 🟡 配置备份恢复弹窗
...
- [ ] 🟡 设计 token 导出 JSON 给工程师
- [ ] 🟡 录制交互动效参考视频
- [ ] 🟡 撰写「设计 → 实现」对照清单（哪些已完成、哪些待补）
